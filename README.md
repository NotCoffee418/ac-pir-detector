...
2. Edit `data/config.json` with your settings:
   ...
   - **pir.pin**: GPIO pin for PIR sensor (default: 33)
   - **pir.detectionCooldownMs**: Cooldown period between detections in milliseconds (default: 30000)
...
# Upload filesystem (config.json) to the board
...
**Note**: Make sure to upload the filesystem (`uploadfs`) before uploading the firmware to ensure your `config.json` is available on the device.
...
- Ensure you uploaded the filesystem with `pio run --target uploadfs`
...
- Ensure you uploaded the filesystem with `pio run --target uploadfs`
- Check serial monitor for "Failed to load config" or "Failed to open config.json" errors
